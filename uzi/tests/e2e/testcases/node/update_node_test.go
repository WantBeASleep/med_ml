...
func (suite *TestSuite) TestUpdateNode_UpdateValidationAINode() {
	data, err := flow.New(
		suite.deps,
		flow.DeviceInit,
		flow.UziInit,
	...
	updateResp, err := suite.deps.Adapter.UpdateNode(
		...
		&pb.UpdateNodeIn{
			Id:         data.Nodes[0].Id.String(),
			Validation: pb.NodeValidation_NODE_VALIDATION_INVALID.Enum(),
		},
	)
	require.NoError(suite.T(), err)
	require.Equal(suite.T(), pb.NodeValidation_NODE_VALIDATION_INVALID, *updateResp.Node.Validation)
}
...
func (suite *TestSuite) TestUpdateNode_UpdateValidationManualNode() {
	data, err := flow.New(
		suite.deps,
		flow.DeviceInit,
		flow.UziInit,
	...
	_, err = suite.deps.Adapter.UpdateNode(
		...
		&pb.UpdateNodeIn{
			Id:         createResp.NodeId,
			Validation: pb.NodeValidation_NODE_VALIDATION_VALID.Enum(),
		},
	)
	require.Error(suite.T(), err)
}
